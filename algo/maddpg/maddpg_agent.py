...
class MADDPG:
    ...
    def save_model(self, file_name, episode):
        ...
        for i in range(self.n_agents):
            torch.save(self.actors[i], file_name + '/actor[' + str(i) + ']' + '_' + str(episode) + '.pth')
            torch.save(self.critics[i], file_name + '/critic[' + str(i) + ']' + '_' + str(episode) + '.pth')

    def update(self, i_episode, total_step):
        ...
        for agent in range(self.n_agents):
            ...
            self.actors[agent].zero_grad()  # are these needed?
            self.critics[agent].zero_grad()  # are these needed?
            ...
            actor_loss = -self.critics[agent](whole_state, whole_action).mean()  # NOTE here, the actor loss does not
    ...
    def choose_action(self, state, cur_total_step, total_training_steps, noise_start_level, noisy=True):
